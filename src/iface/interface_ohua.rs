...
) -> ( OInterfaceInner<'a>, Meta, OhuaTcpSocket<'a>, Result<(bool, bool)>)
where
    DeviceT: for<'d> Device<'d>,
{
    // What we do here
...
impl<'a> OInterfaceInner<'a> {
    ...
    fn process_sixlowpan<'output, 'payload: 'output, T: AsRef<[u8]> + ?Sized>(
        ...
        net_debug!("We currently don't support 6LoWPAN");
        None
        /*
        let payload = match check!(SixlowpanPacket::dispatch(payload)) {
            ...
            SixlowpanPacket::FragmentHeader => {
                ...
                if frag.is_first_fragment() {
                    ...
                    // This information is the total size of the packet when it is fully assembled.
